...
class MsSqlToGCSParquetOperator(BaseOperator):
    """
    Get specified table and persist to parquet in the scratch landing zone bucket
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(MsSqlToGCSParquetOperator, self).__init__(*args, **kwargs)
    ...
    def execute(self, context):
        #pylint: disable=too-many-locals
        mssql = MsSqlHook(mssql_conn_id=self.source_connection)
        mssql_conn = mssql.get_conn()
        ...
        #     )
        #     sql = f"""select * from {self.source_schema}.{self.source_table} where {self.source_filter_column} > TO_TIMESTAMP('{high_watermark}','yyyy-mm-dd hh24:mi:ss.ff6')"""
        # # elif self.sync_method == "INCREMENTAL":
        #     sql = f'''select * from {self.source_schema}.{self.source_table} where {self.source_filter_column} >= TO_DATE('{ds_nodash}', 'YYYYMMDD') and {self.source_filter_column} < TO_DATE('{next_day}', 'YYYYMMDD')'''
        ...
        rows = 0
        for chunk in pd.read_sql_query(sql, mssql_conn, chunksize=chunk_size):
