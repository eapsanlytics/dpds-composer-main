"""
DS APR:  Load heap data into GCS - LATM
June 22, 2023   Added search view.  this was present in apr specific code.
                since querying ingestion prod requires special permissions and
                all data for APR is ingested with these methods, we have added
                search view to the heap ingestion and removed the query from the 
                calling code (search purchase of popularity based models)
"""
...
# should be separated out into it's own ingestion config to avoid cross contamination
...
with DAG('ds_apr_heap_load_LATM_v1',
         description='Workflow to load heap data into gcs for APR - Latin America',
         schedule_interval="25 8 * * 1",
    ...
    COUNTRY_LIST = pd.read_csv(StringIO(Variable.get('DS_APR_HEAP_LATM')), header='infer')
