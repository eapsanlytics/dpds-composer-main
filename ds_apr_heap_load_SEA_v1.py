"""
File:       ds_apr_heap_load_SEA_v1.py
Date:       March 8, 2022
Author:     MJ
Purpose:    Load Heap data for APR model ingestion
Notes:      Composer did not manage parallel gsutil commands well, 
            tasks would start and just hang.  Hypothesis is 
            a competition between resources accessing the shell.
Date:       Feb 21, 2023
            Consolidated the 5 SEA DAGs 
"""
...
with DAG('ds_apr_heap_load_SEA_v1',
         description='Workflow to load heap data into gcs for APR',
    ...
    PDP_VIEW_IDENTITY_TEMPLATE_IN = """bq query --nouse_legacy_sql --project_id={{params.project}} "EXPORT DATA OPTIONS(
        ...
        SELECT
            ...
            COUNT(*) AS count
        FROM
            heap_ecomm_{{params.data_topic}}_production_migrated.product_view_pdp_2
        WHERE
            domain = '{{params.url}}'
            AND DATE(time) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL {{params.num_months}} MONTH) AND CURRENT_DATE()
        GROUP BY
            1,2,3,4,5 ),
        users AS (
        SELECT DISTINCT
    ...
    ADD_TO_CART_IDENTITY_TEMPLATE = """bq query --nouse_legacy_sql --project_id={{params.project}} "EXPORT DATA
        ...
        WHERE
            domain = '{{params.url}}'
            AND DATE(time) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL {{params.num_months}} MONTH)
            AND CURRENT_DATE()
        GROUP BY
            1,2,3,4,5),
        users AS (
    ...
    SEARCH_VIEW_IDENTITY_TEMPLATE = """bq query --nouse_legacy_sql --project_id={{params.project}} "EXPORT DATA OPTIONS(
        ...
        WHERE
            DATE(search.time) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL {{params.num_months}} MONTH)
            AND CURRENT_DATE()
    ...
    for index, row in COUNTRY_LIST.iterrows():
        ...
        if row["country"] == 'india':
             run_pdp_view_identity_command = BashOperator(
                ...
                bash_command=PDP_VIEW_IDENTITY_TEMPLATE_IN,
                params={
                    "country":row["country"],
