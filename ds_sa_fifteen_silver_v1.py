"""
DS Smart Assistant fifteen to silver migration model pipeline
"""
...
DAG_CONFIG = Variable.get('DS_SA_FIFTEEN_SILVER_CONFIG', deserialize_json=True)
...
UPDATE_YML = """cp /home/airflow/gcs/data/mj-migration.pem /tmp; chmod 600 /tmp/mj-migration.pem; ssh -i /tmp/mj-migration.pem -o StrictHostKeyChecking=no michael_janzen_amway_com@{{ ti.xcom_pull(key="recent_ip") }} "sudo -- bash -c 'whoami; cd /migration-home; ls -l; pwd; python3 /migration-home/smart_assist_2.0/migration/fifteen_silver/update_fifteen_silver_yml.py --timestamp """ + TIME_STAMP + """' " """
MIGRATION_BQ = """cp /home/airflow/gcs/data/mj-migration.pem /tmp; chmod 600 /tmp/mj-migration.pem; ssh -i /tmp/mj-migration.pem -o StrictHostKeyChecking=no michael_janzen_amway_com@{{ ti.xcom_pull(key="recent_ip") }} "sudo -- bash -c 'whoami; cd /migration-home; ls -l; pwd; python3 /migration-home/smart_assist_2.0/migration/fifteen_silver/BQ-migration-15toSilver.py --timestamp """ + TIME_STAMP + """' " """
MIGRATION_TEST_SET = """ssh -i /tmp/mj-migration.pem -o StrictHostKeyChecking=no michael_janzen_amway_com@{{ ti.xcom_pull(key="recent_ip") }} "sudo -- bash -c 'whoami; cd /migration-home; ls -l; pwd; python3 /migration-home/smart_assist_2.0/migration/fifteen_silver/BQ-migration-15toSilver-Test-Set.py --timestamp """ + TIME_STAMP + """' " """
MIGRATION_GET_DATA = """ssh -i /tmp/mj-migration.pem -o StrictHostKeyChecking=no michael_janzen_amway_com@{{ ti.xcom_pull(key="recent_ip") }} "sudo -- bash -c 'whoami; cd /migration-home; ls -l; pwd; python3 /migration-home/smart_assist_2.0/migration/fifteen_silver/get_data.py --timestamp """ + TIME_STAMP + """' " """
MIGRATION_TRAIN = """ssh -i /tmp/mj-migration.pem -o StrictHostKeyChecking=no michael_janzen_amway_com@{{ ti.xcom_pull(key="recent_ip") }} "sudo -- bash -c 'whoami; cd /migration-home; ls -l; pwd; python3 /migration-home/smart_assist_2.0/migration/fifteen_silver/train_validate_update.py --timestamp """ + TIME_STAMP + """ --action t ' " """
MIGRATION_SCORE = """ssh -i /tmp/mj-migration.pem -o StrictHostKeyChecking=no michael_janzen_amway_com@{{ ti.xcom_pull(key="recent_ip") }} "sudo -- bash -c 'whoami; cd /migration-home; ls -l; pwd; python3 /migration-home/smart_assist_2.0/migration/fifteen_silver/train_validate_update.py --timestamp """ + TIME_STAMP + """ --action s ' " """
FINAL_CURATION = """ssh -i /tmp/mj-migration.pem -o StrictHostKeyChecking=no michael_janzen_amway_com@{{ ti.xcom_pull(key="recent_ip") }} "sudo -- bash -c 'whoami; cd /migration-home; ls -l; pwd; python3 /migration-home/smart_assist_2.0/migration/fifteen_silver/fifteen_silver_final_curation.py --timestamp """ + TIME_STAMP + """ ' " """
...
with DAG('ds_sa_fifteen_silver_v1',
         description='Workflow to run migration models in GCP - fifteen to silver',
