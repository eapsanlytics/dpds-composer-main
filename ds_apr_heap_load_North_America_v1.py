"""
DS APR:  Load heap data into GCS
"""
...
with DAG('ds_apr_heap_load_North_America_v1',
         description='Workflow to load heap data into gcs for APR - North America (US, CA)',
    ...
    SEARCH_VIEW_IDENTITY_TEMPLATE = """bq query --nouse_legacy_sql --project_id={{params.project}} "EXPORT DATA OPTIONS(
        ...
        SELECT
            ...
            searchkeyword
        FROM
            amw-dna-ingestion-prd.heap_ecomm_{{params.data_topic}}_production_migrated.search_view_results_2 AS SEARCH
        WHERE
