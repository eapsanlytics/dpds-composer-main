"""
DS APR:  Load heap data into GCS for Japan
"""
...
with DAG('ds_apr_heap_load_JP_v1',
    ...
    COUNTRY_LIST = pd.read_csv(StringIO(Variable.get('DS_APR_HEAP_JP')), header='infer')
    ...
    PDP_VIEW_IDENTITY_TEMPLATE = """bq query --nouse_legacy_sql --project_id={{params.project}} "EXPORT DATA OPTIONS(
        ...
        SELECT
            ...
            COUNT(*) AS count
        FROM
            heap_ecomm_{{params.data_topic}}_production_new_migrated.product_view_pdp
        WHERE
            domain = '{{params.url}}'
        ...
        SELECT DISTINCT
            ...
            identity
        FROM
            heap_ecomm_{{params.data_topic}}_production_new_migrated.users )
        SELECT
            day,
            user_id,
            identity,
    ...
    ADD_TO_CART_IDENTITY_TEMPLATE = """bq query --nouse_legacy_sql --project_id={{params.project}} "EXPORT DATA
        ...
        SELECT
            ...
            COUNT(*) AS count
        FROM
            heap_ecomm_{{params.data_topic}}_production_new_migrated.product_click_add_to_cart
        WHERE
            domain = '{{params.url}}'
    ...
    SUBMIT_ORDER_IDENTITY_TEMPLATE = """bq query --nouse_legacy_sql --project_id={{params.project}} "EXPORT DATA
        ...
        SELECT
            ...
            COUNT(*) AS count
        FROM
            heap_ecomm_{{params.data_topic}}_production_new_migrated.product_click_submit_order
        WHERE
            domain = '{{params.url}}'
            AND DATE(time) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 6 MONTH)
            AND CURRENT_DATE()
    ...
    SEARCH_VIEW_IDENTITY_TEMPLATE = """bq query --nouse_legacy_sql --project_id={{params.project}} "EXPORT DATA OPTIONS(
        ...
        SELECT
            ...
            searchkeyword
        FROM
            amw-dna-ingestion-prd.heap_ecomm_{{params.data_topic}}_production_new_migrated.search_view_results AS SEARCH
        WHERE
